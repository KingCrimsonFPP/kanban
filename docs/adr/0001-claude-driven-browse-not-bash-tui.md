...
We want to navigate a kanban board interactively — board overview → open a card → read it → see blockers — with a rich option-picker UX. It must work in **two contexts**: at a local terminal, and driving Claude Code via **remote control on the cloud mobile app**. Environment: Windows + Git Bash (mintty); the plugin is currently zero-dependency bash; `fzf`/`dialog`/`whiptail`/`gum` are not installed and Python `curses` is unavailable on Windows.
...
`AskUserQuestion` is capped at 4 options, so it cannot enumerate an arbitrary-length card list. Navigation therefore uses a **printed markdown board + type-a-card-id + ≤4 targetless action options**.
