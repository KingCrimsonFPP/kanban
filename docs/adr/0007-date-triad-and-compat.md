...
The first cut made start_date→due_date THE range. Same-day use exposed the
flaw: "when I'll do it" and "when it's owed" are different concepts.
...
present = the range reads start→due (first-cut cards keep rendering). The
